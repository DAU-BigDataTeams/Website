...
![D-4](/assets/img/수식%20쪼가리/D-4.png)    
...
![D-5](/assets/img/수식%20쪼가리/D-5.png)  
...
![D-6](/assets/img/수식%20쪼가리/D-6.png)  

여기서 $|E|$ 배달 이동성 그래프에서 관측된 edge의 수다. $DT_{i,j}$는 배달 이동성 그래프의 실제 배달 시간이다.($DˆT_{i,j}$이거는 예측값 즉, 위 식은 L1 Loss 같음) 배달 수용량을 포함하는 edge 임베딩은 지역 유형 heterogeneous multi-graph(이종 다중 그래프)의 입력으로 활용됨  

## E. Heterogeneous Multi-graph based Recommendation
----
직관적으로, **지역이 특정 매장 유형에 적합한지 여부는 주변 고객 선호도에 영향을 받는다.**